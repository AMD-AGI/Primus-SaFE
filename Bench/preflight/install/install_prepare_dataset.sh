...
#
# Pre-download model (config, tokenizer) and data (wikitext, tokenized cache) during Docker build.
# This avoids runtime network downloads, ReadTimeout, and 429 rate limit errors.
#
...
echo "============== begin to pre-download model_check (model + data) =============="
...
if python3 "$SCRIPT_DIR/pre_download_model_check.py"; then
  echo "Pre-download completed: model + data cached at ${CACHE_ROOT}"
else
