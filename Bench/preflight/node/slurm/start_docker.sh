...
GPU_PRODUCT=`rocm-smi --showproductname |grep 'GPU\[0\]'| grep "Card Series" | awk -F" " '{print $NF}'`
if [ -z "$GPU_PRODUCT" ]; then
  GPU_PRODUCT="MI325X"
fi
export GPU_PRODUCT=$GPU_PRODUCT
...
docker_podman_proxy run --rm \
    ...
    $PREFLIGHT_NODE_IMAGE /bin/bash -c "cd $PRIMUS_PATH/node && GPU_PRODUCT=\"$GPU_PRODUCT\" bash run.sh"