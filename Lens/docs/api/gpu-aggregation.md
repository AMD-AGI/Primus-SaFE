...
Retrieves hourly aggregated GPU statistics at the workload level, showing detailed resource allocation and utilization for individual workloads (Jobs, Deployments, StatefulSets, etc.).

**Endpoint:** `GET /api/gpu-aggregation/workloads/hourly-stats`
...
| `cluster` | string | No | Cluster name (uses default if not specified) |
| `namespace` | string | No | Namespace name (returns all namespaces if not specified) |
| `workload_name` | string | No | Workload name (returns all workloads if not specified) |
| `workload_type` | string | No | Workload type filter (e.g., Job, Deployment, StatefulSet, DaemonSet) |
| `start_time` | string | Yes | Start time in RFC3339 format |
| `end_time` | string | Yes | End time in RFC3339 format |
| `page` | integer | No | Page number, starting from 1 |
| `page_size` | integer | No | Items per page, default 20, maximum 1000 |
| `order_by` | string | No | Sort field: `time` or `utilization` |
    ...
    "data": [
      {
        ...
        "avg_utilization": 0.8567,
        "max_utilization": 0.9820,
        "min_utilization": 0.7234,
        "p50_utilization": 0.8512,
        "p95_utilization": 0.9650,
      ...
      },
      {
        ...
        "avg_utilization": 0.6234,
        "max_utilization": 0.8100,
        "min_utilization": 0.4200,
        "p50_utilization": 0.6100,
        "p95_utilization": 0.7850,
...
| `data` | array | Array of workload hourly statistics records |

**Workload Statistics Record Fields:**
...
| `cluster_name` | string | Name of the cluster |
| `namespace` | string | Kubernetes namespace |
| `workload_name` | string | Name of the workload |
| `workload_type` | string | Type of Kubernetes workload (Job, Deployment, StatefulSet, DaemonSet, etc.) |
| `stat_hour` | string | Statistical hour (RFC3339 format) |
| `allocated_gpu_count` | float | Average number of GPUs actually allocated to this workload during this hour |
| `requested_gpu_count` | float | Average number of GPUs requested by this workload during this hour |
| `avg_utilization` | float | Average GPU utilization (0.0 to 1.0) |
| `max_utilization` | float | Maximum GPU utilization (0.0 to 1.0) |
| `min_utilization` | float | Minimum GPU utilization (0.0 to 1.0) |
| `p50_utilization` | float | Median (50th percentile) GPU utilization (0.0 to 1.0) |
| `p95_utilization` | float | 95th percentile GPU utilization (0.0 to 1.0) |
| `avg_gpu_memory_used` | float | Average GPU memory used in GB |
| `max_gpu_memory_used` | float | Maximum GPU memory used in GB |
| `avg_gpu_memory_total` | float | Average total GPU memory in GB |
| `avg_replica_count` | float | Average number of pod replicas running during this hour |
| `max_replica_count` | integer | Maximum number of pod replicas |
| `min_replica_count` | integer | Minimum number of pod replicas |
| `workload_status` | string | Status of the workload (Running, Completed, Failed, Pending, etc.) |
| `sample_count` | integer | Number of samples collected from Prometheus during this hour |
| `owner_uid` | string | UID of the workload owner from metadata |
| `owner_name` | string | Name of the workload owner |
| `labels` | object | Workload labels stored in JSONB format |
| `annotations` | object | Workload annotations stored in JSONB format |
| `created_at` | string | Record creation time (RFC3339 format) |
| `updated_at` | string | Record last update time (RFC3339 format) |
...
```bash
# Get stats for all workloads in a time range
curl -X GET "http://localhost:8080/api/gpu-aggregation/workloads/hourly-stats?start_time=2025-11-05T00:00:00Z&end_time=2025-11-05T23:59:59Z"

# Get stats for workloads in a specific namespace
curl -X GET "http://localhost:8080/api/gpu-aggregation/workloads/hourly-stats?namespace=ml-training&start_time=2025-11-05T00:00:00Z&end_time=2025-11-05T23:59:59Z"

# Get stats for a specific workload
curl -X GET "http://localhost:8080/api/gpu-aggregation/workloads/hourly-stats?namespace=ml-training&workload_name=bert-training-job&start_time=2025-11-01T00:00:00Z&end_time=2025-11-07T23:59:59Z"

# Filter by workload type (e.g., only Jobs)
curl -X GET "http://localhost:8080/api/gpu-aggregation/workloads/hourly-stats?workload_type=Job&start_time=2025-11-05T00:00:00Z&end_time=2025-11-05T23:59:59Z"

# Get stats with pagination and sorting
curl -X GET "http://localhost:8080/api/gpu-aggregation/workloads/hourly-stats?start_time=2025-11-05T00:00:00Z&end_time=2025-11-05T23:59:59Z&page=1&page_size=50&order_by=utilization&order_direction=desc"

# Get stats for Deployments in a specific cluster and namespace
curl -X GET "http://localhost:8080/api/gpu-aggregation/workloads/hourly-stats?cluster=gpu-cluster-01&namespace=production&workload_type=Deployment&start_time=2025-11-01T00:00:00Z&end_time=2025-11-07T23:59:59Z"
```
...
### WorkloadGpuHourlyStats

Represents hourly aggregated GPU statistics at the workload level.
...
type WorkloadGpuHourlyStats struct {
    ...
    WorkloadType      string    // Type of Kubernetes workload (Job, Deployment, etc.)
    StatHour          time.Time // Statistical hour (rounded down)
    AllocatedGpuCount float64   // Average number of GPUs actually allocated
    RequestedGpuCount float64   // Average number of GPUs requested
    AvgUtilization    float64   // Average utilization
    MaxUtilization    float64   // Maximum utilization
    MinUtilization    float64   // Minimum utilization
    P50Utilization    float64   // Median utilization
    P95Utilization    float64   // 95th percentile utilization
    AvgGpuMemoryUsed  float64   // Average GPU memory used in GB
    MaxGpuMemoryUsed  float64   // Maximum GPU memory used in GB
    AvgGpuMemoryTotal float64   // Average total GPU memory in GB
    AvgReplicaCount   float64   // Average number of pod replicas
    MaxReplicaCount   int32     // Maximum number of pod replicas
    MinReplicaCount   int32     // Minimum number of pod replicas
    WorkloadStatus    string    // Workload status (Running, Completed, Failed, etc.)
    SampleCount       int32     // Number of samples collected
    OwnerUID          string    // UID of the workload owner
    OwnerName         string    // Name of the workload owner
    Labels            ExtType   // Workload labels (JSONB)
    Annotations       ExtType   // Workload annotations (JSONB)
    CreatedAt         time.Time // Record creation time
    UpdatedAt         time.Time // Record update time
}
```
...
Analyze individual workload GPU usage and efficiency:

```bash
# Get all workloads sorted by utilization (find underutilized workloads)
curl -X GET "http://localhost:8080/api/gpu-aggregation/workloads/hourly-stats?start_time=2025-11-05T00:00:00Z&end_time=2025-11-05T23:59:59Z&order_by=utilization&order_direction=asc"

# Track a specific training job's GPU usage over time
curl -X GET "http://localhost:8080/api/gpu-aggregation/workloads/hourly-stats?namespace=ml-training&workload_name=bert-training-job&start_time=2025-11-01T00:00:00Z&end_time=2025-11-07T23:59:59Z"

# Compare GPU efficiency across different workload types
curl -X GET "http://localhost:8080/api/gpu-aggregation/workloads/hourly-stats?workload_type=Job&start_time=2025-11-05T00:00:00Z&end_time=2025-11-05T23:59:59Z"

# Monitor production inference services
curl -X GET "http://localhost:8080/api/gpu-aggregation/workloads/hourly-stats?namespace=production&workload_type=Deployment&start_time=2025-11-05T00:00:00Z&end_time=2025-11-05T23:59:59Z"
```
...
5. **Dimension Types**:
   ...
   - `workload`: Per-workload (Job, Deployment, StatefulSet, etc.) aggregation

6. **Workload-Level Statistics**:
   - Provides the most granular view of GPU resource usage
   - Includes detailed metrics like replica counts, memory usage, and workload status
   - Supports filtering by namespace, workload name, and workload type
   - Labels and annotations are stored in JSONB format for flexible querying
   - Useful for identifying inefficient workloads, tracking training job progress, and monitoring service performance
   - The `requested_gpu_count` vs `allocated_gpu_count` comparison helps identify scheduling issues

7. **Snapshots vs Hourly Stats**:
...
8. **Sample Count**: The `sample_count` field in cluster and workload hourly stats indicates how many data points were collected from Prometheus during that hour. A low sample count may indicate data collection issues.

9. **Default Time Range**: When querying snapshots without time parameters, the API defaults to the last 24 hours.

10. **Cluster Selection**: If the `cluster` parameter is not specified, the system uses the default cluster configured in the cluster manager.

11. **Pagination**: The cluster, namespace, label, and workload hourly statistics endpoints support pagination with a maximum page size of 1000 items. Use the `page` and `page_size` parameters to control pagination. The response includes `total`, `page`, `page_size`, and `total_pages` fields.

12. **Metadata Endpoints**: The metadata endpoints (`/clusters`, `/namespaces`, `/dimension-keys`) help you discover available resources before querying detailed statistics. These endpoints are particularly useful for:
...
5. **Workload Queries**:
   - Use pagination for large result sets (many workloads over extended time periods)
   - Filter by `workload_type` to focus on specific types of workloads (Jobs vs Deployments)
   - Sort by `utilization` to identify inefficient or underutilized workloads
   - Compare `requested_gpu_count` vs `allocated_gpu_count` to detect resource allocation issues
   - Use `workload_status` to filter completed, failed, or running workloads
   - Leverage the JSONB `labels` and `annotations` fields for custom queries in your application
...
7. **Performance**:
   ...
   - Use pagination for workload queries when querying large time ranges
