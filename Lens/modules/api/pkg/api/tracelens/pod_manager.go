...
func CreatePodAsync(ctx context.Context, clusterName string, session *cpmodel.TracelensSessions, profilerFilePath string) {
	go func() {
		...
		if err := CreatePod(createCtx, clusterName, session, profilerFilePath); err != nil {
			...
			// Mark session as failed in Control Plane database
			facade := cpdb.GetControlPlaneFacade().GetTraceLensSession()
...
// session metadata is stored in Control Plane database.
func CreatePod(ctx context.Context, clusterName string, session *cpmodel.TracelensSessions, profilerFilePath string) error {
	...
	if err := facade.UpdateStatus(ctx, session.SessionID, cpmodel.SessionStatusCreating, "Creating pod"); err != nil {
	...
	// Build pod spec
	pod := buildPodSpec(ctx, clusterName, session, podName, profilerFilePath)
	...
	if err := facade.UpdateStatus(ctx, session.SessionID, cpmodel.SessionStatusInitializing, "Waiting for pod to be ready"); err != nil {
		return fmt.Errorf("failed to update session status: %w", err)
	}
	...
	log.Infof("TraceLens pod %s is ready with IP %s for session %s (cluster: %s)", podName, podIP, session.SessionID, clusterName)
	return nil
}
...
func buildPodSpec(ctx context.Context, clusterName string, session *cpmodel.TracelensSessions, podName, profilerFilePath string) *corev1.Pod {
	...
	labels := map[string]string{
		...
		"tracelens.lens.primus/cluster":  clusterName,
	}
	...
	var imageURL string
	if imageTag != "" {
		imageURL = registry.GetImageURLForCluster(ctx, clusterName, registry.ImageTraceLens, imageTag)
	} else {
		imageURL = registry.GetDefaultImageURLForCluster(ctx, clusterName, registry.ImageTraceLens)
	}
	log.Debugf("Using TraceLens image: %s for cluster: %s", imageURL, clusterName)
	...
	return &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				...
				"tracelens.lens.primus/cluster-name":  clusterName,
			},
		},
		Spec: corev1.PodSpec{
