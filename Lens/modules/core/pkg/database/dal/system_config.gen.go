...
func newSystemConfig(db *gorm.DB, opts ...gen.DOOption) systemConfig {
	...
	_systemConfig.Value = field.NewField(tableName, "value")
	...
	_systemConfig.Version = field.NewInt32(tableName, "version")
	_systemConfig.IsReadonly = field.NewBool(tableName, "is_readonly")
	_systemConfig.CreatedAt = field.NewTime(tableName, "created_at")
	_systemConfig.UpdatedAt = field.NewTime(tableName, "updated_at")
	_systemConfig.CreatedBy = field.NewString(tableName, "created_by")
	_systemConfig.UpdatedBy = field.NewString(tableName, "updated_by")
...
type systemConfig struct {
	...
	ALL         field.Asterisk
	ID          field.Int64
	Key         field.String // Unique configuration key identifier
	Value       field.Field  // JSONB value containing configuration data
	Description field.String // Human-readable description of the configuration
	Category    field.String // Configuration category for grouping related configs
	IsEncrypted field.Bool   // Flag indicating if the value is encrypted
	Version     field.Int32  // Configuration version for tracking changes
	IsReadonly  field.Bool   // Flag indicating if the configuration is read-only
	CreatedAt   field.Time   // Timestamp when the configuration was first created
	UpdatedAt   field.Time   // Timestamp when the configuration was last updated
	CreatedBy   field.String // User who created the configuration
	UpdatedBy   field.String // User who last updated the configuration
...
func (s *systemConfig) updateTableName(table string) *systemConfig {
	...
	s.IsReadonly = field.NewBool(table, "is_readonly")
