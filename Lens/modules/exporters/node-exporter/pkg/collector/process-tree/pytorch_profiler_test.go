...
func TestIdentifyProfilerFile(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Create a temporary file to satisfy the stat check
			tmpDir := t.TempDir()
			testFile := filepath.Join(tmpDir, "test-file")
			err := os.WriteFile(testFile, []byte("test content with some data"), 0644)
			assert.NoError(t, err)

			// Use the temp file path for identification
			...
			if tt.shouldIdentify {
				...
				if fileInfo != nil {
					...
					// Note: actual file type detection uses the original path pattern
				}
			} else {
				// Test with actual path that shouldn't match
				fileInfo = reader.identifyProfilerFile(12345, "10", tt.filePath)
