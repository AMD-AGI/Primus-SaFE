...
try:
    ...
    _api_enabled = False
    print("[Primus Lens WandB] API reporter not available, using file-based reporting only")
...
class WandbInterceptor:
    ...
    def __init__(self):
        ...
        if self.api_reporting_enabled:
            ...
            print("[Primus Lens WandB] API reporting enabled")
    ...
    def _save_metrics(self, data: Dict[str, Any], step: Optional[int] = None):
        ...
        try:
            ...
            print(f"[Primus Lens WandB] Failed to save metrics: {e}")
    ...
    def _report_framework_detection(self, wandb_run):
        ...
        try:
            ...
            if not detection_data.get("workload_uid"):
                print("[Primus Lens WandB] Warning: WORKLOAD_UID not set, detection may not be associated with workload")
            
            # 异步上报
            self.api_reporter.report_detection(detection_data)
            
            print(f"[Primus Lens WandB] Framework detection data queued for reporting")
            if detection_data.get("hints", {}).get("possible_frameworks"):
                print(f"  Detected frameworks: {detection_data['hints']['possible_frameworks']}")
                print(f"  Confidence: {detection_data['hints']['confidence']}")
        
        except Exception as e:
            print(f"[Primus Lens WandB] Failed to report framework detection: {e}")
            import traceback
            traceback.print_exc()
    ...
    def patch_wandb(self):
        ...
        def intercepted_init(*args, **kwargs):
            """拦截 wandb.init"""
            print("[Primus Lens WandB] Intercepted wandb.init()")
            
            rank_info = self._get_rank_info()
            print(f"[Primus Lens WandB] Rank info: {rank_info}")
            
            # 设置输出路径
            output_path = self._setup_metrics_output()
            if output_path:
                print(f"[Primus Lens WandB] Metrics will be saved to: {output_path}")
            ...
            if result:
                ...
                print(f"[Primus Lens WandB] WandB run initialized: {result.name}")
                print(f"[Primus Lens WandB] Project: {result.project}")
                print(f"[Primus Lens WandB] Run ID: {self.run_id}")
        ...
        print("[Primus Lens WandB] WandB successfully patched!")
    ...
    def install(self):
        ...
        print("[Primus Lens WandB] Installing WandB hook...")
...
def install_wandb_hook():
    ...
    if _global_interceptor is None:
        ...
        def cleanup_on_exit():
            """程序退出时清理资源"""
            print("[Primus Lens WandB] Cleaning up...")
            if _api_enabled:
                shutdown_reporter()
