...
// Old pipeline stage name constants (for backward compatibility, e.g. isInfrastructureStage in main).
// The active pipeline uses the stages/ package (StageV2) and stage names like "operator-pgo", "database-migration".
const (
...
// DefaultStageTimeouts provides default timeouts for each stage type (names must match stages/ package Name()).
var DefaultStageTimeouts = map[string]time.Duration{
	"operator-pgo":              5 * time.Minute,
	"operator-victoriametrics":  5 * time.Minute,
	"operator-opensearch":       5 * time.Minute,
	"operator-grafana":          5 * time.Minute,
	"operator-fluent":           5 * time.Minute,
	"operator-kube-state-metrics": 5 * time.Minute,
...
// Stage is the legacy stage interface (only used by StageAdapter for backward compatibility).
