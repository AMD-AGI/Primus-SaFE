...
func (j *GpuHistoryCache1hJob) Run(ctx context.Context, clientSets *clientsets.K8SClientSet, storageClientSet *clientsets.StorageClientSet) (*common.ExecutionStats, error) {
	...
	span, ctx := trace.StartSpanFromContext(ctx, "gpu_history_cache_1h_job.Run")
	...
	span.SetAttributes(
		attribute.String("job.name", "gpu_history_cache_1h"),
		attribute.String("cluster.name", clusterName),
		attribute.Int("time_range_hours", 1),
	)
...
func (j *GpuHistoryCache1hJob) cacheGpuUsageHistory(
	...
	cacheSpan.SetAttributes(
		attribute.String("cache.key", CacheKeyGpuUsageHistory1h),
