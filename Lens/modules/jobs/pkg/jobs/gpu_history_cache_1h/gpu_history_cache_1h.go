...
func (j *GpuHistoryCache1hJob) Run(ctx context.Context, clientSets *clientsets.K8SClientSet, storageClientSet *clientsets.StorageClientSet) error {
...
func (j *GpuHistoryCache1hJob) cacheGpuUsageHistory(
	...
	if err := cacheFacade.Set(ctx, cacheKey, result, &expiresAt); err != nil {
		return fmt.Errorf("failed to set cache for GPU usage history (1h): %w", err)
	}
