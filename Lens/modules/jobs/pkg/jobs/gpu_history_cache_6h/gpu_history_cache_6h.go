...
func (j *GpuHistoryCache6hJob) Run(ctx context.Context, clientSets *clientsets.K8SClientSet, storageClientSet *clientsets.StorageClientSet) error {
...
func (j *GpuHistoryCache6hJob) cacheGpuUsageHistory(
	...
	cacheKey := fmt.Sprintf("%s:%s", CacheKeyGpuUsageHistory6h, clusterName)
	expiresAt := time.Now().Add(CacheExpirationDuration)

	// Store in cache
	if err := cacheFacade.Set(ctx, cacheKey, result, &expiresAt); err != nil {
		return fmt.Errorf("failed to set cache for GPU usage history (6h): %w", err)
	}
