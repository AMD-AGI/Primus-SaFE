...
// TestFullMatchingFlow tests the complete flow from JSON configuration to log matching
func TestFullMatchingFlow(t *testing.T) {
	// 1. Simulate JSON configuration loaded from database (matching JSON in SQL file)
	jsonConfig := `{
	...
	// Fixed configuration (using only two backslashes, which become one after JSON parsing)
	jsonConfigFixed := `{
	...
	// Test original configuration (with bug)
...
// testPatternConfig tests a single configuration
