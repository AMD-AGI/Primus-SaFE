...
func TestParseLogQuery(t *testing.T) {
	...
	tests := []struct {
		...
		beginTime time.Time
		endTime   time.Time
		validate  func(*testing.T, *types.ListLogRequest, error)
	}{
		{
    ...
    "keywords": ["key1", "key2"],
			...
			beginTime: time.Time{},
			endTime:   time.Time{},
			validate: func(t *testing.T, query *types.ListLogRequest, err error) {
...
func TestBuildSearchBody(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			query: &types.ListLogRequest{
				ListLogInput: types.ListLogInput{
					Offset: 0,
					Limit:  100,
					Order:  dbclient.ASC,
				},
...
func TestAddContextDoc(t *testing.T) {
	tests := []struct {
		name      string
		query     types.ListContextLogRequest
