...
func (h *Handler) BatchCreateTasks(c *gin.Context) {
	...
	for i := range req.Items {
		resp, err := h.submitTask(c.Request.Context(), &req.Items[i], userID, userName, "", h.clawBearerForGin(c))
		if err != nil {
...
func (h *Handler) ListArtifacts(c *gin.Context) {
	...
	clawCtx := WithClawBearer(c.Request.Context(), h.clawBearerForGin(c))
...
func (h *Handler) DownloadArtifact(c *gin.Context) {
	...
	data, err := h.clawClient.ReadSessionFile(WithClawBearer(c.Request.Context(), h.clawBearerForGin(c)), task.ClawSessionID, path)
	if err != nil {
