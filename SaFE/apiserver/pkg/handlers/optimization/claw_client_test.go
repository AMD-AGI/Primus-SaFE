...
// --- merged from claw_client_http_test.go ---
...
func TestClawCreateSessionWithMessage(t *testing.T) {
	...
	testifyassert.True(t, res.Dispatched != nil && *res.Dispatched)
}
...
func TestClawCreateSessionWithMessageNoDispatchedField(t *testing.T) {
	...
	testifyassert.NoError(t, err)
	assert.Equal(t, "sess-1", res.SessionID)
	testifyassert.Nil(t, res.Dispatched)
}
...
func TestCreateClawSessionWithRetryTransientFailures(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		testifyassert.NoError(t, json.NewDecoder(r.Body).Decode(&body))
...
func TestClawSendMessage(t *testing.T) {
	...
	testifyassert.Error(t, c.SendMessage(context.Background(), "", &MessageRequest{}))
}
...
func TestClawGetSession(t *testing.T) {
	...
	testifyassert.True(t, ss.IsTerminal())

	testifyassert.Error(t, func() error { _, e := c.GetSession(context.Background(), ""); return e }())
}

func TestClawDeleteSession(t *testing.T) {
	c, _ := newClawTestServer(t)
	testifyassert.NoError(t, c.DeleteSession(context.Background(), "sess-1"))
	// Empty id is a no-op (no error).
	testifyassert.NoError(t, c.DeleteSession(context.Background(), ""))
}

func TestClawInterruptSession(t *testing.T) {
	c, _ := newClawTestServer(t)
	testifyassert.NoError(t, c.InterruptSession(context.Background(), "sess-1"))
	testifyassert.Error(t, c.InterruptSession(context.Background(), ""))
}
...
func TestClawListSessionFiles(t *testing.T) {
	...
	testifyassert.NoError(t, err)
	testifyassert.Len(t, files, 1)
	testifyassert.Contains(t, files[0].Path, "optimization_report.md")
...
func TestClawStream(t *testing.T) {
	...
	// Empty session id -> error.
	testifyassert.Error(t, c.Stream(context.Background(), "", "", func(ClawSSEEvent) error { return nil }))
}
