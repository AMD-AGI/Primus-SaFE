...
func TestCvtToFaultResponseItem(t *testing.T) {
	...
	tests := []struct {
		...
		validate func(*testing.T, view.FaultResponseItem)
	}{
		{
			...
			validate: func(t *testing.T, result view.FaultResponseItem) {
...
func TestParseListFaultQuery(t *testing.T) {
	...
	tests := []struct {
		...
		validate    func(*testing.T, *view.ListFaultRequest, error)
	}{
		{
			name:        "empty query uses defaults",
			queryParams: map[string]string{},
			validate: func(t *testing.T, query *view.ListFaultRequest, err error) {
...
// --- merged from fault_handlers_test.go ---
