...
func InitCustomRouters(e *gin.Engine, h *Handler) {
	...
	group := e.Group(common.PrimusRouterCustomRootPath, middle.Authorize(), middle.Preprocess())
	{
		...
		group.POST(fmt.Sprintf("nodes/:%s/retry", common.Name), h.RetryNode)
		...
		group.POST(fmt.Sprintf("workloads/:%s/logs/:%s/context", common.Name, view.DocId), h.GetWorkloadLogContext)
