...
func (h *Handler) createSecret(c *gin.Context) (interface{}, error) {
	...
	return &view.CreateSecretResponse{
...
func (h *Handler) createSecretImpl(ctx context.Context, req *view.CreateSecretRequest, requestUser *v1.User) (*corev1.Secret, error) {
...
func (h *Handler) listSecret(c *gin.Context) (interface{}, error) {
	...
	result := &view.ListSecretResponse{}
...
func (h *Handler) authSecretUpdate(c *gin.Context, req *view.PatchSecretRequest, secret *corev1.Secret) error {
	requestUser, err := h.getAndSetUsername(c)
	if err != nil {
		return err
	}
...
func applySecretPatch(secret *corev1.Secret, req *view.PatchSecretRequest) error {
	if req.Params != nil {
...
func generateSecret(req *view.CreateSecretRequest, requestUser *v1.User) (*corev1.Secret, error) {
...
func buildSecretData(reqType v1.SecretType, allParams []map[view.SecretParam]string, secret *corev1.Secret) error {
	var secretType corev1.SecretType
	data := make(map[string][]byte)

	switch reqType {
	case v1.SecretImage:
		keys := []view.SecretParam{view.PasswordParam, view.UserNameParam, view.ServerParam}
		secretType = corev1.SecretTypeDockerConfigJson
		dockerConf := view.DockerConfig{}
		dockerConf.Auths = make(map[string]view.DockerConfigItem)
		for _, params := range allParams {
			...
			password := stringutil.Base64Decode(params[view.PasswordParam])
			auth := stringutil.Base64Encode(fmt.Sprintf("%s:%s", params[view.UserNameParam], password))
			dockerConf.Auths[params[view.ServerParam]] = view.DockerConfigItem{
				UserName: params[view.UserNameParam],
				Password: password,
				Auth:     auth,
			}
		}
		data[view.DockerConfigJson] = jsonutils.MarshalSilently(dockerConf)
	case v1.SecretSSH:
		if len(allParams) == 0 {
			return fmt.Errorf("the input params are empty")
		}
		params := allParams[0]
		if !existKey(params, view.UserNameParam) {
			return fmt.Errorf("the %s is empty", view.UserNameParam)
		}
		secretType = corev1.SecretTypeOpaque
		data[string(view.UserNameParam)] = []byte(params[view.UserNameParam])
		if val, _ := params[view.PasswordParam]; val != "" {
			data[string(view.PasswordParam)] = []byte(stringutil.Base64Decode(params[view.PasswordParam]))
		} else if existKey(params, view.PublicKeyParam) && existKey(params, view.PrivateKeyParam) {
			data[view.SSHAuthKey] = []byte(stringutil.Base64Decode(params[view.PrivateKeyParam]))
			data[view.SSHAuthPubKey] = []byte(stringutil.Base64Decode(params[view.PublicKeyParam]))
		} else {
...
func existKey(params map[view.SecretParam]string, key view.SecretParam) bool {
	val, _ := params[key]
	return val != ""
}
...
func buildSecretLabelSelector(query *view.ListSecretRequest) labels.Selector {
...
func cvtToSecretResponseItem(secret *corev1.Secret) view.SecretResponseItem {
	result := view.SecretResponseItem{
		SecretId:     secret.Name,
...
func cvtToGetSecretResponse(secret *corev1.Secret) view.GetSecretResponse {
	...
	case string(v1.SecretImage):
		dockerConf := &view.DockerConfig{}
		if json.Unmarshal(secret.Data[view.DockerConfigJson], dockerConf) == nil {
			result.Params = make([]map[view.SecretParam]string, 0, len(dockerConf.Auths))
			for k, v := range dockerConf.Auths {
				params := make(map[view.SecretParam]string)
				params[view.ServerParam] = k
				params[view.UserNameParam] = v.UserName
				params[view.PasswordParam] = stringutil.Base64Encode(v.Password)
				result.Params = append(result.Params, params)
			}
		}
	case string(v1.SecretSSH):
		result.Params = make([]map[view.SecretParam]string, 0, 1)
		params := make(map[view.SecretParam]string)
		params[view.UserNameParam] = string(secret.Data[string(view.UserNameParam)])
		if pwd, ok := secret.Data[string(view.PasswordParam)]; ok && len(pwd) > 0 {
			params[view.PasswordParam] = stringutil.Base64Encode(string(pwd))
		} else {
			if auth := secret.Data[view.SSHAuthKey]; len(auth) > 0 {
				params[view.PrivateKeyParam] = stringutil.Base64Encode(string(auth))
			}
			if authPub := secret.Data[view.SSHAuthPubKey]; len(authPub) > 0 {
				params[view.PublicKeyParam] = stringutil.Base64Encode(string(authPub))
			}
		}
		result.Params = append(result.Params, params)
	case string(v1.SecretGeneral):
		result.Params = make([]map[view.SecretParam]string, 0, len(secret.Data))
		params := make(map[view.SecretParam]string)
		for k, v := range secret.Data {
			params[view.SecretParam(k)] = stringutil.Base64Encode(string(v))
		}
		result.Params = append(result.Params, params)
	}
	return result
...
func parseCreateSecretRequest(c *gin.Context) (*view.CreateSecretRequest, error) {
