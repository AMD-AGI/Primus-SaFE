...
func (h *Handler) createUser(c *gin.Context) (interface{}, error) {
	...
	return &view.CreateUserResponse{Id: user.Name}, nil
}
...
func generateUser(req *view.CreateUserRequest) *v1.User {
	user := &v1.User{
...
func (h *Handler) listUser(c *gin.Context) (interface{}, error) {
	...
	result := view.ListUserResponse{}
	if len(userList.Items) > 0 {
		sort.Sort(view.UserSlice(userList.Items))
	}
...
func applyUserPatch(targetUser *v1.User, req *view.PatchUserRequest) {
...
func (h *Handler) authUserUpdate(c *gin.Context, targetUser *v1.User, req *view.PatchUserRequest) (bool, error) {
	requestUser, err := h.getAndSetUsername(c)
	if err != nil {
		return false, err
	}
...
func setCookie(c *gin.Context, userInfo *view.UserLoginResponse, userType v1.UserType) {
	maxAge := 0
	switch {
...
func (h *Handler) cvtToUserResponseItem(ctx context.Context, user *v1.User) view.UserResponseItem {
	result := view.UserResponseItem{
		Id:             user.Name,
	...
	if !user.IsSystemAdmin() {
		...
		for _, id := range workspaces {
			...
			result.Workspaces = append(result.Workspaces, view.WorkspaceEntry{
		...
		for _, id := range workspaces {
			...
			result.ManagedWorkspaces = append(result.ManagedWorkspaces, view.WorkspaceEntry{
				Id: id, Name: v1.GetDisplayName(workspace),
			})
		}
	}
...
func parseLoginQuery(c *gin.Context) (*view.UserLoginRequest, error) {
	req := &view.UserLoginRequest{}
...
func parseListUserQuery(c *gin.Context) (*view.ListUserRequest, error) {
	query := &view.ListUserRequest{}
...
func buildListUserSelector(query *view.ListUserRequest) labels.Selector {
