...
// The idle roles reach the workload through inferaOptions on the patch, which
...
func TestApplyWorkloadPatchSetsInferaIdleRoles(t *testing.T) {
	workload := &v1.Workload{}
	req := &view.PatchWorkloadRequest{
		InferaOptions: &view.DynamoOptions{
			IdleRoles: []string{"prefill", "decode"},
		},
	}

	assert.NilError(t, applyWorkloadPatch(workload, req))
	assert.Equal(t,
		v1.GetAnnotation(workload, v1.InferaIdleRolesAnnotation), "prefill,decode")
}

// Correcting a role that was wrongly marked idle has to be expressible. An
// empty slice is a request to clear, distinct from omitting inferaOptions.
func TestApplyWorkloadPatchClearsInferaIdleRoles(t *testing.T) {
	workload := &v1.Workload{}
	v1.SetAnnotation(workload, v1.InferaIdleRolesAnnotation, "prefill,decode")

	req := &view.PatchWorkloadRequest{
		InferaOptions: &view.DynamoOptions{IdleRoles: []string{}},
	}
	assert.NilError(t, applyWorkloadPatch(workload, req))
	assert.Equal(t, v1.GetAnnotation(workload, v1.InferaIdleRolesAnnotation), "")
}

// A patch that does not mention inferaOptions must not disturb it -- an image
// upgrade is sent that way and must not disturb the idle marking.
func TestApplyWorkloadPatchLeavesInferaOptionsAloneWhenAbsent(t *testing.T) {
	workload := &v1.Workload{}
	v1.SetAnnotation(workload, v1.InferaIdleRolesAnnotation, "decode")
