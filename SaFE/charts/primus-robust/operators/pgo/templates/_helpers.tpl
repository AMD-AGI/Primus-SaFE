...
{{- $prefix := include "pgo.imagePrefix" . -}}
{{- range $id, $object := .Values.relatedImages }}
- name: RELATED_IMAGE_{{ $id | upper }}
  value: {{ printf "%s%s" $prefix $object.image | quote }}
{{- end }}
{{- end }}

{{- define "install.controllerImage" -}}
{{- printf "%s%s" (include "pgo.imagePrefix" .) (required ".Values.controllerImages.cluster is required" .Values.controllerImages.cluster) -}}
{{- end -}}
