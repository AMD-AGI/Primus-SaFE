...
func TestMetricsRemoteWriteGettersDefaults(t *testing.T) {
	viper.Reset()
	assert.False(t, IsMetricsRemoteWriteEnabled())
	assert.Equal(t, "", GetMetricsRemoteWriteURL())
	assert.Equal(t, 30, GetMetricsRemoteWriteIntervalSeconds())
	assert.Equal(t, "primus-safe", GetMetricsRemoteWriteJob())
	assert.Equal(t, "", GetMetricsRemoteWriteClusterName())
	assert.False(t, IsMetricsRemoteWriteInsecureSkipVerify())
}

func TestMetricsRemoteWriteGettersWithValues(t *testing.T) {
	viper.Reset()
	viper.Set(metricsRemoteWriteEnable, true)
	viper.Set(metricsRemoteWriteURL, "https://vm.example/insert/0/prometheus")
	viper.Set(metricsRemoteWriteInterval, 60)
	viper.Set(metricsRemoteWriteJob, "primus-safe")
	viper.Set(metricsRemoteWriteClusterName, "crusoe")
	viper.Set(metricsRemoteWriteInsecureSkipVerify, true)

	assert.True(t, IsMetricsRemoteWriteEnabled())
	assert.Equal(t, "https://vm.example/insert/0/prometheus", GetMetricsRemoteWriteURL())
	assert.Equal(t, 60, GetMetricsRemoteWriteIntervalSeconds())
	assert.Equal(t, "crusoe", GetMetricsRemoteWriteClusterName())
	assert.True(t, IsMetricsRemoteWriteInsecureSkipVerify())
}

func TestMetricsRemoteWriteIntervalMinimum(t *testing.T) {
	viper.Reset()
	viper.Set(metricsRemoteWriteInterval, 1) // below the 5s floor
	assert.Equal(t, 5, GetMetricsRemoteWriteIntervalSeconds())
}

func TestMetricsRemoteWriteClusterNameFallsBackToSubDomain(t *testing.T) {
	viper.Reset()
	viper.Set(subDomain, "tw325")
	assert.Equal(t, "tw325", GetMetricsRemoteWriteClusterName())
	viper.Set(metricsRemoteWriteClusterName, "crusoe")
	assert.Equal(t, "crusoe", GetMetricsRemoteWriteClusterName())
}
