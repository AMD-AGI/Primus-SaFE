...
func TestIsPathAccessibleFromWorkspace_MultipleVolumes(t *testing.T) {
	...
	k8sClient := ctrlfake.NewClientBuilder().WithScheme(scheme).WithObjects(ws).Build()
...
// --- merged from workspace_extra_test.go ---
...
func TestGetWorkspacesWithSamePath(t *testing.T) {
	...
	testifyassert.ElementsMatch(t, []string{"a", "b"}, got)
}
