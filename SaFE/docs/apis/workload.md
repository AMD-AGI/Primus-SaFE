...
```json
{
  ...
  "images": ["harbor.example.com/ai/pytorch:2.0","harbor.example.com/ai/pytorch:2.0"],
  "entryPoints": ["cHl0aG9uIHRyYWluLnB5","cHl0aG9uIHRyYWluLnB5"],
  "priority": 0,
  "timeout": 3600,
  "maxRetry": 0,
  "env": {
...
```json
{
  ...
  "images": ["harbor.example.com/ai/infer:1.0"],
...
***TorchFT Request Example***:
```json
{
  "displayName": "my-torchFT-job",
  "description": "TorchFT job description",
  "workspaceId": "cluster-workspace",
  "groupVersionKind": {
    "kind": "TorchFT",
    "version": "v1"
  },
  "images": ["harbor.example.com/ai/torchft:2.0","harbor.example.com/ai/torchft:2.0"],
  "entryPoints": ["cHl0aG9uIHRyYWluLnB5","cHl0aG9uIHRyYWluLnB5"],
  "resources": [{
    "cpu": "2",
    "memory": "4Gi",
...
| images                       | []string | No       | Image address. The length of the `images` array should match the length of the `resources` array to ensure that each resource has a corresponding image configuration.                                    |
| entryPoints                  | []string | No       | Startup command/script (Base64 encoded). The length of the `entryPoints` array should match the length of the `resources` array to ensure that each resource has a corresponding entryPoint configuration. |
...
| forceHostNetwork             | bool     | No       | Whether to use host network forcibly. Default logic relies on task size: it triggers only for multi-node tasks using full GPU allocation.                                                    |
...
| kind | string | No | Filter by type: Deployment/PyTorchJob/StatefulSet/Authoring/AutoscalingRunnerSet/TorchFT (comma-separated)                     |
...
| groupVersionKind.kind        | string | Workload type: PyTorchJob/Deployment/StatefulSet/Authoring/AutoscalingRunnerSet/TorchFT  |
...
| resources[].cpu              | string | CPU cores, e.g. "128"                                                                    |
| resources[].gpu              | string | GPU cards, e.g. "8"                                                                      |
| resources[].memory           | string | Memory size, e.g. "256Gi"                                                                |
...
| Field                           | Type       | Description                                                                                                                             |
|---------------------------------|------------|-----------------------------------------------------------------------------------------------------------------------------------------|
| images                          | []string   | Image address used by the workload                                                                                                      |
| entryPoints                     | []string   | Startup command/script (in base64 encoding)                                                                                             |
...
| nodesHistory[].truncated        | boolean    | Whether older dispatches or ranks were removed to fit the history size limit                                                             |
...
```json
{
  ...
  "images": ["harbor.example.com/ai/pytorch:2.1","harbor.example.com/ai/pytorch:2.1"],
  "entryPoints": ["YmFzaCBydW4uc2gK","YmFzaCBydW4uc2gK"],
  "priority": 1,
...
| resources           | []obeject | No | A complete list of resources requirements for a workload.        |
| images              | []string  | No | Image address             |
| entryPoints         | []string  | No | Startup command/script (Base64 encoded)                          |
| description         | string    | No | Workload description                                             |
...
| TorchFT              | Fault-tolerant distributed training   | Deep learning training |
