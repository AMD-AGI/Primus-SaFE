...
type TcpFlow struct {
	Saddr   [28]byte `ebpf:"saddr"`
	Daddr   [28]byte `ebpf:"daddr"`
	SPort   uint16   `ebpf:"sport"`
	DPort   uint16   `ebpf:"dport"`
	Family  uint16   `ebpf:"family"`
	Reason  uint16   `ebpf:"reason"`
	DataLen uint32   `ebpf:"data_len"`
	Srtt    uint32   `ebpf:"srtt"`
	Pid     uint32   `ebpf:"pid"`
}

func (c TcpFlow) String() string {
	return fmt.Sprintf("Family %d Saddr %s Daddr %s SPort %d DPort %d DataLen %d Reason %d Srtt %d Pid %d",
		c.Family, c.GetSaddr(), c.GetDaddr(), c.SPort, c.DPort, c.DataLen, c.Reason, c.Srtt, c.Pid)
}
	...
	for {
		...
		default:
			...
			if err != nil {
				...
				slog.Error("failed to read from ringbuf", "error", err)
				continue
			}
			var event TcpFlow
			...
			slog.Debug("Received tcpflow event", "event", event.String())
...
func (b *BpfTcpFlow) init() error {
	...
	if err := b.initProbeLink(objs); err != nil {
		slog.Error("failed to init probe link", "error", err)
		return err
	}
	return nil
}
