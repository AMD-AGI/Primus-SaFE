...
func workloadMapper(obj *unstructured.Unstructured) *dbclient.Workload {
	...
	// Offloaded workloads store per-pod detail and dispatch history directly in
	// the workload_pod / workload_dispatch_node tables via the job-manager syncer,
	// so they are not mirrored into the row columns. Legacy (pre-offload) workloads
	// still live on etcd, so keep mirroring their status arrays so the apiserver
	// detail read can fall back to these columns and stay fresh.
	if !v1.IsWorkloadStatusOffloadEnabled(workload) {
...
func modelMapper(obj *unstructured.Unstructured) *dbclient.Model {
	...
	dbModel := &dbclient.Model{
		...
		TargetVolume:  cr.Spec.TargetVolume,
		TargetSubpath: cr.Spec.TargetSubpath,
