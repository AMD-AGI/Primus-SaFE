...
func TestWorkloadFilter(t *testing.T) {
	...
	// nil inputs -> not filtered.
	assert.Equal(t, false, workloadFilter(nil, u2))
	// identical spec/status -> filtered out.
...
func TestSetupExportersDBDisabled(t *testing.T) {
	// When DB is disabled SetupExporters returns nil without touching the manager.
...
func TestWorkloadMapperRichBranches(t *testing.T) {
	...
	// Deletion timestamp set -> phase forced to Stopped.
