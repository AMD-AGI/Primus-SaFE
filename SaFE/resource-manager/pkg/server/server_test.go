...
// --- merged from server_init_test.go ---
...
func TestNewServerWithStubs(t *testing.T) {
	...
	s, err := NewServer()
	testifyassert.NoError(t, err)
	testifyassert.NotNil(t, s)
	testifyassert.True(t, s.isInited)
}
...
func TestServerInitLogsAndConfig(t *testing.T) {
	...
	testifyassert.NoError(t, s.initLogs())
	testifyassert.NoError(t, s.initConfig())
}
