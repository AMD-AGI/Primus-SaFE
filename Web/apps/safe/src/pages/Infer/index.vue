<template>
  <el-text class="block textx-18 font-500" tag="b">Infer</el-text>
    ...
    <div class="table-wrap">
      ...
      >
        ...
        <el-table-column prop="replica" label="Replicas" min-width="90">
          <template #default="{ row }">
            {{ row.resources?.[0]?.replica ?? row.resource?.replica ?? 0 }}
          </template>
        </el-table-column>
...
const jumpToDetail = (id: string) => {
  router.push({ path: '/infer/detail', query: { id } })
}
...
const fetchData = async (params?: WorkloadParams) => {
  try {
    ...
    const res = await getWorkloadsList({
      ...
      kind: `${WorkloadKind.Deployment},${WorkloadKind.StatefulSet}`,
      ...params,
    })
