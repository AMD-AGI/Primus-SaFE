...
describe('inferaPayload', () => {
  it('uses the Infera defaults image by default', () => {
  ...
  it('uses kv-aware as the default router policy and allows round-robin', () => {
    ...
    const payload = buildInferaCreatePayload(form, 'core42-hyperloom')

    const frontendEntrypoint = decodeFromBase64String(payload.entryPoints[0])
    expect(frontendEntrypoint).toBe(
      'python3 -m infera.server --host 0.0.0.0 --port 8000 --router-policy round-robin --router-tokenizer-path /wekafs/models/DeepSeek-R1-0528',
    )
    expect(frontendEntrypoint).not.toContain('--discovery-backend')
    expect(frontendEntrypoint).not.toContain('--request-transport')
  })

  it('builds a single-node Infera payload with the standard worker entrypoint', () => {
    const form = createDefaultInferaForm()
    form.displayName = 'infera-ds-r1-single'
    ...
    expect(payload.inferaOptions).toEqual({
  ...
  it('builds an aggregation Infera payload with frontend and worker roles', () => {
    const form = createDefaultInferaForm()
    form.displayName = 'infera-ds-r1-agg-nats'
    ...
    expect(payload.groupVersionKind).toEqual({ kind: 'InferaDeployment', version: 'v1' })
    expect(payload.images).toEqual([form.image, form.image])
    expect(payload.resources).toEqual([
      { replica: 1, cpu: '4', memory: '16Gi' },
      {
    ...
    const workerEntrypoint = decodeFromBase64String(payload.entryPoints[1])
    expect(workerEntrypoint).toContain('python3 -m infera.engine.sglang')
  ...
  it('builds a PD Infera payload with role-specific entrypoints and mori KV backend', () => {
    const form = createDefaultInferaForm()
    ...
    expect(prefillEntrypoint).toContain('python3 -m infera.engine.vllm')
    expect(decodeEntrypoint).toContain('python3 -m infera.engine.sglang')
  ...
  it('uses custom Infera role entrypoints independently', () => {
    const form = createDefaultInferaForm()
    form.enablePd = true
    form.prefillEntrypoint = 'python3 -m infera.engine.sglang --model-path /models/prefill\n'
    form.decodeEntrypoint = 'python3 -m infera.engine.sglang --model-path /models/decode\n'
  ...
  it('uses a custom Infera frontend entrypoint when provided', () => {
    const form = createDefaultInferaForm()
    form.frontendEntrypoint = 'python3 -m infera.server --host 0.0.0.0 --port 9000 --router-tokenizer-path /models/custom\n'
