<template>
  <el-text class="block textx-18 font-500" tag="b">TorchFT</el-text>
    ...
    <div class="table-wrap">
      ...
      >
          ...
          <template #default="{ row }">
            <div class="flex flex-col items-start">
              <div class="flex items-center">
                <el-link
                  type="primary"
                  v-route="{ path: '/torchft/detail', query: { id: row.workloadId } }"
                >{{
                  row.displayName
                }}</el-link>
                <el-tooltip
        ...
        <el-table-column prop="replica" label="Replicas" min-width="90">
          <template #default="{ row }">
            {{ row.resources?.[1]?.replica || row.resource?.replica || 0 }}
          </template>
        </el-table-column>
        ...
        <el-table-column prop="resource" label="Resource" min-width="280">
          <template #default="{ row }">
            <span class="res-line">
              <span class="t">{{ (row.resources?.[1] || row.resource)?.gpu ?? 0 }} card</span>
              <span class="sep">*</span>
              <span class="t">{{ (row.resources?.[1] || row.resource)?.cpu }} core</span>
              <span class="sep">*</span>
              <span class="t">{{ (row.resources?.[1] || row.resource)?.memory }}</span>
            </span>
          </template>
        </el-table-column>
