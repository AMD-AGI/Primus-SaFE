<template>
    ...
    <el-tab-pane label="Overview" name="overview">
      ...
      <el-card class="mt-6 safe-card" shadow="never">
        ...
        <div class="grid gap-3 mt-4 sm:grid-cols-2 lg:grid-cols-3">
          <StatCard label="Replicas" :value="totalReplica" :icon="DataLine" />
  ...
  <AddDialog
    ...
    :wlid="workloadId"
    :action="addAction"
    @success="addAction === 'Edit' ? getDetail() : router.push('/training')"
  />
  <SshConfigDialog
...
const totalReplica = computed(() => {
  if (!detailData.value?.resources || !Array.isArray(detailData.value.resources)) {
    return '-'
  }
  const sum = detailData.value.resources.reduce((acc: number, resource: WorkloadResource) => {
    return acc + (resource?.replica ?? 0)
  }, 0)
  return sum || '-'
})
