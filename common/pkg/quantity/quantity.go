...
func CvtToResourceList(cpu, memory, gpu, gpuType, ephemeralStore string, replica int64) (corev1.ResourceList, error) {
	...
	if cpu != "" {
		...
		if cpuQuantity.Value() <= 0 {
			return nil, commonerrors.NewBadRequest("invalid cpu")
		}
	...
	if memory != "" {
		...
		if memQuantity.Value() <= 0 {
			return nil, commonerrors.NewBadRequest("invalid memory")
		}
	...
	if gpu != "" && gpuType != "" {
		...
		if gpuQuantity.Value() <= 0 {
			return nil, commonerrors.NewBadRequest("invalid gpu")
		}
	...
	if ephemeralStore != "" {
		...
		if ephemeralStoreQuantity.Value() <= 0 {
			return nil, commonerrors.NewBadRequest("invalid ephemeral store")
		}
...
// Calculate available resources: available = total - reserved
