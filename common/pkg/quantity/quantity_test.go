...
func TestSubResource(t *testing.T) {
	...
	gpu, ok := result[common.NvidiaGpu]
...
func TestCvtToResourceList(t *testing.T) {
	resource, err := CvtToResourceList("1000m", "512", "8", common.NvidiaGpu, "", 2)
	assert.NilError(t, err)
	assert.Equal(t, resource.Cpu().Value(), int64(2))
	assert.Equal(t, resource.Memory().Value(), int64(1024))
	gpu, ok := resource[common.NvidiaGpu]
	assert.Equal(t, ok, true)
	assert.Equal(t, gpu.Value(), int64(16))
}

func TestGetReserveResource(t *testing.T) {
	...
	commonconfig.SetValue("workspace.mem_reserve_percent", "0.05")
	commonconfig.SetValue("workspace.cpu_reserve_percent", "0.08")
	defer func() {
		commonconfig.SetValue("workspace.mem_reserve_percent", "")
		commonconfig.SetValue("workspace.cpu_reserve_percent", "")
	}()
	resource = GetAvailableResource(resource)

	q, ok := resource[corev1.ResourceCPU]
	assert.Equal(t, ok, true)
	assert.Equal(t, q.Value(), int64(920))

	q, ok = resource[corev1.ResourceMemory]
	assert.Equal(t, ok, true)
	assert.Equal(t, q.Value(), int64(950))

	q, ok = resource[common.NvidiaGpu]
	assert.Equal(t, ok, true)
	assert.Equal(t, q.Value(), int64(8))
}
