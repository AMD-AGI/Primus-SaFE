...
func SetupWorkloadTTLController(mgr manager.Manager) error {
	...
	err := ctrlruntime.NewControllerManagedBy(mgr).
		For(&v1.Workload{}, builder.WithPredicates(CaredTTLChangePredicate{})).
		Complete(r)
	if err != nil {
		return err
	}
...
type CaredTTLChangePredicate struct {
	predicate.Funcs
}

func (CaredTTLChangePredicate) Create(e event.CreateEvent) bool {
...
func (CaredTTLChangePredicate) Update(e event.UpdateEvent) bool {
...
func (r *WorkloadTTLController) handle(ctx context.Context, workload *v1.Workload) (ctrlruntime.Result, error) {
	...
	case workload.IsEnd():
		...
		if workload.Status.EndTime != nil {
			costTime = int(nowTime.Sub(workload.Status.EndTime.Time).Seconds())
		}
		if costTime >= ttlSeconds {
			err = r.deleteWorkload(ctx, workload)
		} else {
			result.RequeueAfter = time.Duration(ttlSeconds-costTime) * time.Second
		}
...
func (r *WorkloadTTLController) addTimeoutCondition(ctx context.Context, workload *v1.Workload) error {
	...
	if err := r.Status().Patch(ctx, workload, patch); err != nil {
		klog.ErrorS(err, "failed to patch workload phase", "name", workload.Name)
		return err
	}
	klog.Infof("the workload %s has timed out. timeout: %d", workload.Name, *workload.Spec.Timeout)
	return nil
}
