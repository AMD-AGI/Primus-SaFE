#!/bin/bash
# OCI Test: 单机全量 SFT (8B, 8 GPU, 1 node)
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
JOB_NAME="oci-test-single-full-$(date +%s)"
echo "========== OCI Test: Single-Node Full SFT =========="
echo "Job: $JOB_NAME"
...
modules:
  post_trainer:
    ...
    overrides:
      ...
      eval_interval: 30
      save_interval: 10
      finetune_lr: 0.0000050000
      min_lr: 0
      lr_warmup_iters: 5
      precision_config: bf16_mixed
EXPEOF

mkdir -p "./output/${PRIMUS_TEAM:-amd}/${PRIMUS_USER:-root}/oci-test-single-full"

prepare_squad_cache
patch_torch_inductor_duplicate_assert

./runner/primus-cli direct -- train posttrain --config /tmp/sft_experiment.yaml
post_train_cleanup "none"
export_model "none" "$JOB_NAME"
