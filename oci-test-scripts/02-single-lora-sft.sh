#!/bin/bash
# OCI Test: 单机 LoRA SFT (8B, 8 GPU, 1 node)
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
JOB_NAME="oci-test-single-lora-$(date +%s)"
echo "========== OCI Test: Single-Node LoRA SFT =========="
echo "Job: $JOB_NAME"
...
mkdir -p "./output/${PRIMUS_TEAM:-amd}/${PRIMUS_USER:-root}/oci-test-single-lora"
